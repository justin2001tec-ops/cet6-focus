...
if (!curation.pairReject.some((candidate) => candidate.word === word && candidate.sentenceId === provenanceEntry.sentenceId)) {
  curation.pairReject.push({ word, sentenceId: provenanceEntry.sentenceId, categories: ['violence', 'political-heavy'], reason: 'The sentence frames the target in an unspecified revolution and violence-related political context rather than a neutral default example.' })
}
...
console.log('Added the current violent-revolution sentence-read reject (idempotent).')
