...
if (!curation.pairReject.some((candidate) => candidate.word === word && candidate.sentenceId === provenanceEntry.sentenceId)) {
  curation.pairReject.push({ word, sentenceId: provenanceEntry.sentenceId, categories: ['political-heavy', 'proper-noun-heavy'], reason: 'The sentence is a geopolitically specific statement about military equipment and China, not a neutral default context.' })
}
...
console.log('Added the current military-equipment sentence-read reject (idempotent).')
