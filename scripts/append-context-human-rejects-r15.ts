...
if (!curation.pairReject.some((candidate) => candidate.word === word && candidate.sentenceId === provenanceEntry.sentenceId)) {
  curation.pairReject.push({ word, sentenceId: provenanceEntry.sentenceId, categories: ['violence', 'public-controversy'], reason: 'The sentence combines a sensitive gender-identity comparison with violent crime and is not a neutral default context.' })
}
...
console.log('Added the current violent-crime identity sentence-read reject (idempotent).')
