...
if (!curation.pairReject.some((candidate) => candidate.word === word && candidate.sentenceId === provenanceEntry.sentenceId)) {
  curation.pairReject.push({ word, sentenceId: provenanceEntry.sentenceId, categories: ['violence', 'political-heavy', 'public-controversy'], reason: 'The sentence is a politically charged characterization of a named public figure and antifascist violence.' })
}
...
console.log('Added the current Orwell sentence-read reject (idempotent).')
