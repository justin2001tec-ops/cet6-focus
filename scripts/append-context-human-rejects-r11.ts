...
if (!curation.pairReject.some((candidate) => candidate.word === word && candidate.sentenceId === provenanceEntry.sentenceId)) {
  curation.pairReject.push({ word, sentenceId: provenanceEntry.sentenceId, categories: ['political-heavy', 'public-controversy'], reason: 'The sentence makes a politically charged accusation about military recruitment and indoctrination.' })
}
...
console.log('Added the current military-recruitment sentence-read reject (idempotent).')
