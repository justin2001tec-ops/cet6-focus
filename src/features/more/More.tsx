...
export function More() {
  ...
  return <div className="page page--more">
    ...
    <section className="more-list" aria-label="更多入口">
      {entries.map(({ to, icon: Icon, title, description }) => <button key={to} type="button" className="ios-list-row" onClick={() => navigate(to)}><span className="ios-list-row__icon"><Icon size={18} /></span><span><strong>{title}</strong><small>{description}</small></span><span className="ios-list-row__chevron">›</span></button>)}
