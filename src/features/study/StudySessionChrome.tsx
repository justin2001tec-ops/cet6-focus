...
export function StudySessionChrome({ modeLabel, progress, index, total, completed, canUndo, onUndo, onHelp, onExit }: StudySessionChromeProps) {
  return (
    <header className="learning-topbar study-session-chrome" data-functional-layer="session-chrome">
      <GlassIconButton label="退出学习" variant="regular" className="study-session-chrome__close" onClick={onExit}><X size={18} strokeWidth={1.8} /></GlassIconButton>
      ...
      <div className="learning-topbar__actions study-session-chrome__actions">
        {canUndo && <GlassSurface as="button" type="button" variant="regular" interactive className="learning-undo" onClick={onUndo}><Undo2 size={15} /><span>撤销</span></GlassSurface>}
        <GlassIconButton label="学习操作帮助" variant="regular" className="study-session-chrome__help" onClick={onHelp}><HelpCircle size={18} strokeWidth={1.8} /></GlassIconButton>
      </div>
    </header>
  )
}
