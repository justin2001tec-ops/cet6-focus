...
test.describe('v1.5 quality hardening', () => {
  ...
  test('Settings import exposes one keyboard-accessible trigger', async ({ page }, testInfo) => {
    ...
    expect(await fileInput.evaluate((node) => (node as HTMLInputElement).hidden)).toBe(true)

    const [keyboardFileChooser] = await Promise.all([page.waitForEvent('filechooser'), trigger.press('Enter')])
    expect(keyboardFileChooser).toBeTruthy()
    const [pointerFileChooser] = await Promise.all([page.waitForEvent('filechooser'), trigger.click()])
    expect(pointerFileChooser).toBeTruthy()
